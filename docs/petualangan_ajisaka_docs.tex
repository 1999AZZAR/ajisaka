...
per-segment angle buckets (e.g., 8 bins). 3. \textbf{Raster Coverage}
(Geometric Outline Matcher) distance to the reference normalized stroke
(proxy: resampled dense polylines calculating intersection over expected
perimeter area). 4. \textbf{Order = truth:} strokes must be completed in
the reference order; a reversal triggers the ``wrong direction''
feedback even if the shape matches. 5. Score →
...
Recognition latency & \textless{} 50 ms (Raster Coverage on 0.02 step
resample) \\
\end{longtable}
...
\textbf{M1 --- Stroke engine} & Canvas capture, guide animation, Raster
Coverage matcher, feedback mapping \\
...
\section{Naskah Aplikasi Petualangan
Ajisaka}\label{naskah-aplikasi-petualangan-ajisaka}
...
dinobatkan menjadi \textbf{Raja Kerajaan Nusantara}.

\section{Petualangan Ajisaka --- Todo Pengembangan
(P0--P3)}\label{petualangan-ajisaka-todo-pengembangan-p0p3}
